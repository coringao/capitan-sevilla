...
/* The purpose of this util is to build encode hints for lattice
   ...
   Threshold hints are for books in which each scalar in the vector
   ...
   sequential book (or can be used along with a sparse threshold map,
   like a splitting tree that need not be trained) 

   If the input book is non-sequential, a threshold hint is built.
...
int main(int argc,char *argv[]){
  ...
  if(c->q_sequencep==0){
    /* yes. Discard any preexisting threshold hint */
  ...
  if(c->q_sequencep || i<entries){
    ...
    the error variance is *not* dependent.  Given a pigeonhole and an
